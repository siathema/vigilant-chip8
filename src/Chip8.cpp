...
void Chip8::run_instruction(uint16_t opcode, bool debug) {
  ...
  if((opcode&0xF000)==0x0000 && (opcode!=0x00E0) && (opcode!=0x00EE)) { // SYS addr - jump to subroutine at addr
    ...
    if(reg_content == op_byte) {
      ...
      if(debug==true) {
	std::cout<<std::setbase(16)<<": SE "<<reg_index<< ", "<<(int)op_byte<<std::endl;
      } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": SNE "<<reg_index<< ", "<<(int)op_byte<<std::endl;
    }   
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": SE "<<reg_index_x<< ", "<<reg_index_y<<std::endl;
    }   
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": LD "<<reg_index<< ", "<<(int)byte<<std::endl;
    }   
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": ADD "<<reg_index<< ", "<<(int)byte<<std::endl;
    }   
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": LD "<<reg_index<< ", "<<((opcode&0x00F0)>>4)<<std::endl;
    }   
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": OR "<<reg_index<< ", "<<((opcode&0x00F0)>>4)<<std::endl;
    }   
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": AND "<<reg_index<< ", "<<((opcode&0x00F0)>>4)<<std::endl;
    }   
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": XOR "<<reg_index<< ", "<<((opcode&0x00F0)>>4)<<std::endl;
    }   
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": ADD "<<reg_index<< ", "<<((opcode&0x00F0)>>4)<<std::endl;
    }   
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": SUB "<<reg_index<< ", "<<((opcode&0x00F0)>>4)<<std::endl;
    }   
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": SHR "<<reg_index<<", "<<((opcode&0x00F0)>>4)<<std::endl;
    }   
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": SUBN "<<reg_index<< ", "<<((opcode&0x00F0)>>4)<<std::endl;
    }   
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": SNE "<<reg_index<< ", "<<((opcode&0x00F0)>>4)<<std::endl;
    } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": LD "<< "I, "<<(opcode&0x0FFF)<<std::endl;
    } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": JP "<< "V0, "<<addr<<std::endl;
    } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": RND "<<reg_index<< ", "<<(opcode&0x00FF)<<std::endl;
    } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": DRW "<<((opcode&0x0F00)>>8)<<", "<<reg_index<<", "<<(int)sprite_index<<std::endl;
    } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": SKP "<<((opcode&0x0F00)>>8)<<std::endl;
    } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": SKNP "<<((opcode&0x0F00)>>8)<<std::endl;
    } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": LD "<<reg_index<<", DT"<<std::endl;
    } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": LD "<<reg_index<<", K"<<std::endl;
    } 
  }
    ...
    if(debug==true) {
      std::cout<<std::setbase(16)<<": LD "<<"DT, "<<reg_index<<std::endl;
    } 
  }
  ...
  else if((opcode & 0xF0FF) == 0xF065) { // LD Vx, [I] - store the memory starting at I in regs[V0..Vx]
...
uint8_t Chip8::get_mem(uint16_t address) {

  return mem[address];

}

uint8_t Chip8::get_V_regs(int index) {
  return DATA_reg[index];
}
...
uint16_t Chip8::get_SP() {

  return SP_reg;

}

uint8_t Chip8::get_timer() {

  return TIMER_reg;

}

uint8_t Chip8::get_sound() {
...
void Chip8::print_state() {
  ...
  for(int i=0; i<16; i++) {
    ...
    std::cout<<"V"<<i<<": "<<std::setbase(16)<<(int)DATA_reg[i]<<std::endl; 
